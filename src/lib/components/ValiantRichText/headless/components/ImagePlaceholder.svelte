<script lang="ts">
	...
	function handleFileChange(e: Event) {
		...
		if (file.size > MAX_FILE_SIZE) {
			uploadError = `File is too large (max ${MAX_FILE_SIZE / 1024 / 1024} MB).`;
			return;
		}
	...
	{#if editor.isEditable}
		<div class="edra-media-placeholder-wrapper edra-media-placeholder-tabs">
			<div class="edra-media-placeholder-icon-wrap">
				...
				<span class="edra-media-placeholder-label">Image</span>
			</div>
			...
			{#if activeTab === 'upload'}
				...
				{#if uploadError}
					<span class="edra-media-placeholder-error">{uploadError}</span>
				{/if}
				<input
					bind:this={fileInput}
					type="file"
